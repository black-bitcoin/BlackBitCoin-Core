...
to complete. Your PC or VM must have at least 1.5 GB of RAM or blackbitcoin will 
...
Install dependencies needed to compile blackbitcoin: 
--------------------- 
...
--------------------
mkdir $HOME/blackbitcoin
cd $HOME/blackbitcoin
git clone https://github.com/mammix2/blackbitcoin-core.git
cd blackbitcoin-core/src/leveldb
...
Qt GUI wallet build + extra dependencies: (if you have already built the daemon you wont need to clone the blackbitcoin repository)
--------------------
cd $HOME
mkdir $HOME/deps
cd $HOME/deps
...
cd blackbitcoin-core/src/leveldb
chmod +x ./build_detect_platform
cd $HOME/blackbitcoin/blackbitcoin-core
qmake "RELEASE=1"
make -j$NPROC
